...
for selfTrainingIteration in range(101):
	...
	for model_number in range(len(randomseeds)):
		...
		if(selfTrainingIteration == 0):
			...
			predictor = Predictor(testSet.shape[1], num_classes).cuda()
			predictor.load_state_dict(torch.load('saved_model_info/'+configfilename+'/SelfTraining/{}Iteration/predictor{}.pt'.format(selfTrainingIteration, model_number)))
			predictor.eval()
	...
	np.save(filename_roc_curve_info+'/'+configfilename+'_SelfTraining{}Iter.npy'.format(selfTrainingIteration), fpr_and_tprs)
